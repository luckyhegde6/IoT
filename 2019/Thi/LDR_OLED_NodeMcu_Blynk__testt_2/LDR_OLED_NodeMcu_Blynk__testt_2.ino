...
void setup() {
  Serial.begin(9600);
  scheduler.schedule(Updatething,50000);; //for multitask -- updating the thingspeak
...
void loop() {
  scheduler.schedule(Updatething,5000);; //for multitask -- updating the thingspeak
  Blynk.run();
...
void Updatething() {
 Serial.println("Value sent to Thingspeak");
  ...
  display.setCursor(32, 10);
   display.println("Value Updated in Thingspeak");
    display.display();
  display.clearDisplay();
  delay(200);
   // IMPORTANT:
