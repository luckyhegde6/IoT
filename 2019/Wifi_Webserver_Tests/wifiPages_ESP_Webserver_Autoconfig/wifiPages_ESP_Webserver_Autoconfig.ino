...
/*#ifdef USE_DHCP_FOR_IP_ADDRESS
    WiFi.begin(mac);  // Use DHCP to get an IP address
#else
    WiFi.begin(mac, ip);
#endif*/
 Serial.begin(115200);
...
MethodType readHttpRequest(WiFiClient & client, int & nUriIndex, BUFFER & requestContent)
{
...
MethodType readRequestLine(WiFiClient & client, BUFFER & readBuffer, int & nUriIndex, BUFFER & requestContent)
{
  MethodType eMethod;
...
void readRequestHeaders(WiFiClient & client, BUFFER & readBuffer, int & nContentLength, bool & bIsUrlEncoded)
{
...
void readEntityBody(WiFiClient & client, int nContentLength, BUFFER & content)
{
  int i;
  char c;
...
***********************************************************************************************************************/
void getNextHttpLine(WiFiClient & client, BUFFER & readBuffer)
{
  char c;
...
*
***********************************************************************************************************************/
void sendPage(WiFiClient & client, int nUriIndex, BUFFER & requestContent)
{
  // send HTML header
...
*                                                              Send Images
***********************************************************************************************************************/
void sendImage(WiFiClient & client, int nUriIndex, BUFFER & requestContent)
{
...
void sendProgMemAsString(WiFiClient & client, const char *realword)
{
  sendProgMemAsBinary(client, realword, strlen_P(realword));
}

// Non-string data needs to provide an explicit size
void sendProgMemAsBinary(WiFiClient & client, const char* realword, int realLen)
{
...
void sendUriContentByIndex(WiFiClient client, int nUriIndex, BUFFER & requestContent)
{
...
void sendSubstitute(WiFiClient client, int nUriIndex, int nSubstituteIndex, BUFFER & requestContent)
{
  if (nUriIndex < NUM_PAGES)
  {
    // Page request
