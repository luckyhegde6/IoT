...
const byte ledPin = LED_BUILTIN;               //LED on pin 13
...
void loop(){
  ...
  if (Serial.available()){            //if we have recieved anything on the Serial
    scheduler.schedule(setHigh,50000);  //schedule a setHigh call in 500 milliseconds
...
void setHigh(){
  ...
  Serial.print("LED Set High");//set ledPin HIGH
  scheduler.schedule(setLow,5000);     //schedule setLow to execute in 500 milliseconds
}

void setLow(){
  digitalWrite(ledPin,LOW);           //set ledPin LOW
  Serial.print("LED Set LOW");//set ledPin LOW
}
