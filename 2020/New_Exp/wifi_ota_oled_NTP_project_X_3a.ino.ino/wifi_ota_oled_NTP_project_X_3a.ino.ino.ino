...
#endif
static int RED_LED = 12;
static int GREEN_LED = 13;
static int BLUE_LED = 14;
//WiFiUDP ntpUDP;
...
//  Serial.println(timeClient.getFormattedTime());
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,10);
  display.println("packet received, length=");
  display.println(cb);
  display.display();
  delay(1000);
  display.clearDisplay();
...
unsigned long sendNTPpacket(IPAddress& address)
{
  ...
  display.setTextColor(WHITE);
  display.setCursor(0,10);
  display.println("sending NTP packet...");
  display.display();
  delay(1000);
  display.clearDisplay();
